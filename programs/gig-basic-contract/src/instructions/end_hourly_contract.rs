...
pub fn end_hourly_contract(
    ctx: Context<EndHourlyContractContext>,
    contract_id: String,
) -> Result<()> {
    msg!("Ending hourly contract on buyer side!");
    ...
    msg!("Ended hourly contract successfully!");
    Ok(())
}

#[derive(Accounts)]
#[instruction(contract_id: String)]
pub struct EndHourlyContractContext<'info> {
    #[account(mut)]
